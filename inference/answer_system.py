...
**提示：** 在回答中引用文献时，**必须使用检索结果中已标记的编号**，不要自己重新编号。**不要生成参考文献列表**。

## 引用编号规则（严格遵守）：

* **重要**：检索内容中的每篇文献已经标记了编号 [1], [2], [3]...（见下方检索内容）
* **引用时必须使用文献在检索结果中的原始编号**，而不是按引用顺序重新编号
* 例如：如果检索结果中第3篇文献标记为 [3]，你在答案中引用它时也必须使用 [3]，不能改为 [1]
...
如果答案中先引用第3篇，再引用第1篇，应该写：
"糖尿病并发症需要特别注意[3]，同时遵循诊断标准[1]..."

**错误示例：**
"糖尿病并发症需要特别注意[1]，同时遵循诊断标准[2]..." ← 错误！应该使用原始编号 [3] 和 [1]
...
## 问题如下："""
            ...
            if not citations_sent and pre_generated_citations:
                # 提取答案中实际使用的引用编号（这些编号对应检索结果中的原始编号）
                import re
                ...
                logger.info(f"📚 答案中引用的编号: {sorted(citation_numbers)}")
                logger.info(f"📚 预生成的参考文献ID: {[c['id'] for c in pre_generated_citations]}")
                
                # 只保留答案中实际引用的文献（按答案中的引用顺序，但保持原始ID）
                used_citations = []
                # 创建ID到citation的映射
                id_to_citation = {c['id']: c for c in pre_generated_citations}
                
                # 按答案中引用的顺序（citation_numbers的第一次出现顺序）收集citations
                # 但保持原始的ID不变
                for num in sorted(citation_numbers):  # 按编号排序，确保顺序一致
                    if num in id_to_citation:
                        used_citations.append(id_to_citation[num])
                    else:
                        logger.warning(f"⚠️ 答案中引用了编号 [{num}]，但检索结果中没有对应的文献（ID范围: 1-{len(pre_generated_citations)}）")
                
                logger.info(f"📚 最终使用的参考文献数量: {len(used_citations)}/{len(pre_generated_citations)}")
                logger.info(f"📚 最终使用的参考文献ID: {[c['id'] for c in used_citations]}")
                
                # 构造完整的answer_data（只包含实际使用的citations，保持原始ID）
                answer_data = {
                    "answer": answer_text.strip(),
                    "citations": used_citations  # 保持原始ID，与检索结果中的编号对应
                }
